...
namespace _56_60_bandienthoai
{
    public partial class frmHOADONBAN : Form
    {
        ...
        public void capNhatCBOMAKHKhiMoThemKH()
        {
            loadComboBox(cboMaKH, dsMaKH, "select * from KHACHHANG", "hoTen", "maKH");
        }

        private void frmCTHOADONBAN_Load(object sender, EventArgs e)
        {
            Console.WriteLine("AAAAAAAAA");
            danhsach_datagridview(dgvCTHDBan, "select *, CONVERT(INT, SUBSTRING(maHD, 4, LEN(maHD)-3)) AS ColPhu  from dbo.CTHOADONBAN  ORDER BY ColPhu DESC;", ref dsCTHDB);
            danhsach_datagridview(dgvHDBan, "select *, CONVERT(INT, SUBSTRING(maHD, 4, LEN(maHD)-3)) AS ColPhu from HOADONBAN ORDER BY ColPhu DESC;", ref dsHDB);
            loadPhimChucNang(true);

            //dsMaHD = c.layDuLieu("select * from HOADONBAN");
        ...
        private void frmCTHOADONBAN_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void btnMoi_Click(object sender, EventArgs e)
        {
            ...
            flag = 1;
            themMoi(dsHDB, txtMaHD, dgvHDBan, "maHD", "HDB", 3);
            txtNgayBan.Text = DateTime.Now.ToString("yyyy-MM-dd");
        }
            ...
            try
            {
                ...
                if (flag == 1 && co == true && dgvDSSP.RowCount > 0)
                {
                    ...
                    if (txtMaHD.Text != "")
                    {
                        ...
                        sql = "insert into HOADONBAN values('" + txtMaHD.Text + "','" + cboMaNV.SelectedValue + "'," +
                            "'" + cboMaKH.SelectedValue + "','" + txtNgayBan.Text + "','" + txtTongTien.Text + "'," +
                            "'" + txtGhiChu.Text + "', '"+cboTrangThai.SelectedIndex.ToString()+ "',0)";
                        ...
                        try
                        {
                            ...
                            foreach (DataGridViewRow row in dgvDSSP.Rows)
                            {
                                if (!row.IsNewRow)
                                {
                                    sql = "insert into CTHOADONBAN values('" + row.Cells["maHD"].Value.ToString() + "'," +
        ...
        {
            if (e.RowIndex >= 0 && e.RowIndex < dgvCTHDBan.Rows.Count - 1)
            {
                int stt = e.RowIndex;
                ...
                danhsach_datagridview(dgvHDBan, "select * from HOADONBAN where maHD = '"+txtMaHD.Text+"'", ref dsHDB);
            }
        }
                        ...
                        case 1:
                            ...
                            try
                            {
                                if (co == true)
                                {
                                    ...
                                    if (txtMaHD.Text != "")
                                    {
                                        foreach (DataGridViewRow row in dgvCTHDBan.Rows)
                                        {
                                            if (!row.IsNewRow)
                                            {
                                                if (row.Cells["maHD"].Value.ToString() == txtMaHD.Text)
                                                {
                                                    ...
                                                    sql = "update CTSANPHAM set soLuong = '" + soLuongSPConLai + "' where maSP = '" + row.Cells["maSP"].Value.ToString() + "'";
                                                    c.capNhatDuLieu(sql);
                                                    sql = "delete from CTHOADONBAN where maHD = '" + txtMaHD.Text + "'";
                                        ...
                                        try
                                        {
                                            sql = "delete from HOADONBAN where maHD = '" + txtMaHD.Text + "'";
                            ...
                            try
                            {
                                if (co == true)
                                {
                                    ...
                                    if (txtMaHD.Text != "")
                                    {
                                        ...
                                        DataSet dsTongTien = c.layDuLieu("select * from HOADONBAN where maHD = '" + txtMaHD.Text + "'");
                                        ...
                                        sql = "update HOADONBAN set tongTien = '" + tongTien + "' where maHD = '" + txtMaHD.Text + "'";
                                        Console.WriteLine("update tongTien: " + sql);
                                        c.capNhatDuLieu(sql);

                                        sql = "delete CTHOADONBAN where maHD = '" + txtMaHD.Text + "' and maSP = '" + cboMaSP.SelectedValue.ToString() + "'";
                        ...
                        try
                        {
                            if (co == true)
                            {
                                ...
                                if (txtMaHD.Text != "")
                                {
                                    ...
                                    sql = "update HOADONBAN set maNV = '"+cboMaNV.SelectedValue.ToString()+"'," +
                                        " ngayBan = '"+txtNgayBan.Text+"', tongTien = '"+txtTongTien.Text+"', trangThai = '"+cboTrangThai.SelectedIndex.ToString()+"' " +
                        ...
                        try
                        {
                            if (co == true)
                            {
                                ...
                                if (txtMaHD.Text != "")
                                {
                                    ...
                                    sql2 = "update CTHOADONBAN set soLuong = '" + txtSoLuong.Text + "', donGia = '" + txtDonGia.Text + "', " +
        ...
        private void txtSoLuong_TextChanged(object sender, EventArgs e)
        {
            ...
            if (txtDonGia.Text != "" && txtSoLuong.Text != "")
            {
                ...
                {
                    DataSet dsSoLuong = c.layDuLieu("select * from CTSANPHAM where maSP = '"+cboMaSP.SelectedValue+"'");
        ...
        private void frmHOADONBAN_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void btnTim_Click(object sender, EventArgs e)
        {
            ...
            {
                dsMaKH = null;
                dsMaKH = c.layDuLieu("select * from KHACHHANG where dienThoai = '" + txtSDT.Text + "'");
                if(dsMaKH != null && dsMaKH.Tables[0].Rows.Count > 0)
                {
                    cboMaKH.Text = dsMaKH.Tables[0].Rows[0]["hoTen"].ToString();
                }
                else
                {
        ...
        private void btnThemDSSP_Click(object sender, EventArgs e)
        {
            ...
            // Câu lệnh đưa 1 đối tượng vào dataGridView
            //object[] d = new object[9];
            //dgvDSSP.Rows.Add(d); 

            //hD.nhapHD(txtMaHD.Text);
            //hD.themVaoDS(cboMaSP.Text,Convert.ToInt32(txtSoLuong.Text), Convert.ToInt32(txtDonGia.Text), Convert.ToInt32(txtThanhTien.Text), txtGhiChu.Text);
            //if(hD.lstHD != null)
            //{
            //    dgvDSSP.DataSource = hD.lstHD;
            //}
            //else
            //{
        ...
        private void btnLocTheoMaNV_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    danhsach_datagridview(dgvCTHDBan, "select b.* from HOADONBAN a, CTHOADONBAN b where a.maHD = b.maHD and a.maNV = '" + cboMaNV.SelectedValue.ToString() + "'", ref dsCTHDB);
                    danhsach_datagridview(dgvHDBan, "select * from HOADONBAN where maNV = '" + cboMaNV.SelectedValue.ToString() + "'", ref dsHDB);
                }
            }
            catch (Exception ex)
            {
        ...
        public void loadcboKhachHangKhiThemMoi()
        {
            loadComboBox(cboMaKH, dsMaKH, "select * from KHACHHANG", "hoTen", "maKH");
        }
        ...
        {
            danhsach_datagridview(dgvCTHDBan, "select * from dbo.CTHOADONBAN", ref dsCTHDB);
            danhsach_datagridview(dgvHDBan, "select * from HOADONBAN", ref dsHDB);
        }

        private void cboMaKH_SelectedIndexChanged(object sender, EventArgs e)
        {
            //if (co == true && cboMaKH.Text != "")
            //{
            //    danhsach_datagridview(dgvCTHDBan, "select c.* from HOADONBAN a, KHACHHANG b, CTHOADONBAN c where a.maKH = b.maKH and a.maHD = c.maHD and b.maKH = '" + cboMaKH.SelectedValue.ToString() + "'", ref dsCNCTHDTHEOKH);
            //    danhsach_datagridview(dgvHDBan, "select a.* from HOADONBAN a, KHACHHANG b where a.maKH = b.maKH and b.maKH = '" + cboMaKH.SelectedValue.ToString() + "'", ref dsCNHDTHEOKH);
            //}
        }

        private void dgvHDBan_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.RowIndex < dgvHDBan.Rows.Count - 1)
            {
                int stt = e.RowIndex;
                txtMaHD.Text = dsHDB.Tables[0].Rows[stt]["maHD"].ToString();
                cboMaNV.SelectedValue = dsHDB.Tables[0].Rows[stt]["maNV"].ToString();
                cboMaKH.SelectedValue = dsHDB.Tables[0].Rows[stt]["maKH"].ToString();
                txtNgayBan.Text = dsHDB.Tables[0].Rows[stt]["ngayBan"].ToString();
                ...
                danhsach_datagridview(dgvCTHDBan, "select * from CTHOADONBAN where maHD = '"+txtMaHD.Text+"'", ref dsCTHDB);
            }
        }
