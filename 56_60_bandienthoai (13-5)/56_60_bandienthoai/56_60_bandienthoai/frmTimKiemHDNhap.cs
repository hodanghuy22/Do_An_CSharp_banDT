...
namespace _56_60_bandienthoai
{
    public partial class frmTimKiemHDNhap : Form
    {
        ...
        {
            dsSP = c.layDuLieu("select * from HOADONNHAP a, CTHOADONNHAP b where a.maHD = b.maHD");
            dgvHDN.DataSource = dsSP.Tables[0];
            ds = c.layDuLieu("select * from HOADONNHAP a, CTHOADONNHAP b where a.maHD = b.maHD");

            cboMa.Enabled = true;
            loadComboBox(cboMa, dsSP, "select maHD from HOADONNHAP group by maHD", "maHD", "maHD");
            flag = true;
        }
        ...
        private void cboMa_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (flag == true)
            {
                if (cboMa.SelectedIndex != -1)
                {
                    ...
                    DataSet laySP = c.layDuLieu("select * from HOADONNHAP a, CTHOADONNHAP b where a.maHD = b.maHD and a.maHD = '" + ma + "'");
                    dgvHDN.DataSource = laySP.Tables[0];
                }
            }
        }
        ...
        {
            if (e.RowIndex >= 0 && e.RowIndex < dgvHDN.Rows.Count - 1)
            {
